...
class StreamingTranscriber:
    ...
    def transcribe_pcm(self, pcm_data: np.ndarray) -> str:
        ...
        Mock mode runs ONLY when USE_MOCK_WHISPER=true was set before launch;
        a runtime failure never fabricates transcript text.
        """
        if os.environ.get("USE_MOCK_WHISPER") == "true":
        ...
        try:
            ...
            # Never substitute the mock scam script here: fabricated text in a
            # live session is indistinguishable from real caller speech and can
            # raise a false SCAM verdict on a genuine call. Fail loud, skip cycle.
            print(f"[ERROR] Whisper transcription failed ({e}). Returning empty text for this cycle.")
            return ""
