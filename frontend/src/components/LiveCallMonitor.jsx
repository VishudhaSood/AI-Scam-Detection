        ...
        {status === 'stopping' ? (
          ...
          <div className="glass-panel" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 style={{ margin: 0 }}>Live Risk Guardian</h3>
              <span style={{
                fontFamily: 'var(--font-mono)', 
                fontSize: '0.85rem',
                fontWeight: 600,
                background: update && update.label === 'SCAM' ? 'rgba(239, 68, 68, 0.15)' : update && update.label === 'SUSPICIOUS' ? 'rgba(245, 158, 11, 0.15)' : 'rgba(16, 185, 129, 0.15)',
                color: update && update.label === 'SCAM' ? 'var(--color-scam, #ef4444)' : update && update.label === 'SUSPICIOUS' ? 'var(--color-suspicious, #f59e0b)' : 'var(--color-safe, #10b981)',
                padding: '0.25rem 0.6rem',
                borderRadius: '0.375rem',
                border: '1px solid currentColor'
              }}>
                {update ? `${update.label} (${Math.round(update.risk_smoothed * 100)}%)` : "SAFE (10%)"}
              </span>
            </div>

            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-around', gap: '1.25rem', flexWrap: 'wrap', background: 'rgba(0,0,0,0.15)', padding: '1rem', borderRadius: '0.5rem', border: '1px solid rgba(255,255,255,0.05)' }}>
              <div style={{ flex: '0 0 auto' }}>
                <RiskGauge score={update ? update.risk_smoothed : 0.10} />
              </div>
              ...
              {update && update.evidence_breakdown && (
                <div style={{ flex: '1 1 200px', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                    ...
                    <span style={{ fontWeight: 700, color: 'var(--accent-purple, #8b5cf6)' }}>{Math.round((update.overall_confidence || 0) * 100)}%</span>
                  </div>
                </div>
              )}
            </div>
